...
**Смена сети у администратора:** сессия администратора при смене подсети (`/24` IPv4, `/64` IPv6) **не сбрасывается** — IP не является фактором аутентификации, а смена сети (мобильный интернет, VPN, поездка) для админа обычное дело. Вместо отказа в доступе пишется событие `ADMIN_IP_CHANGE` в `critical.log` (уровень LOW), которое попадает в суточный дайджест и в плитку «Admin IP Change» панели «Безопасность» на `/admin`. После записи IP сессии обновляется, поэтому одна смена сети даёт одну запись. Если смена сети не ваша — нажмите «Выйти со всех устройств» и войдите заново. Ограничение: при использовании Tailscale Funnel (`start_ubuntu_tailscale_funnel.sh`) событие не сработает, так как uvicorn запускается без `--proxy-headers` и все клиенты видны с одного адреса прокси.
